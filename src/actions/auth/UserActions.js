...
import { storeItem, getItem, removeItem,clearDB,clearStorage   } from "@/utils/AsyncUtils";
...
export const editSubscriptionsList = (navigation,id,cb) => {
  return (dispatch) => {
    dispatch(loadingAction(true));
    UserController.getAuthCall(API.SUBSCRIPTION_EDIT+`/?productSubscriptionId=${id}`)
      .then((res) => {
        dispatch(loadingAction(false));
        if (res.messageID == 200) {   
          if(cb){
