...
import { getCartItems, addToWish, delCartItem, delWish,addToCart, getJikapuCartItems } from "@/actions";
import { COLORS } from "@/constants";
import { strings } from "@/localization";
import { jikapu, redHeart, heart, defaultImage, searchIcon1 } from "@/assets";

export const JFreshCart = ({ route, navigation }) => {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        >
          { jFreshData !== null &&  jFreshData?.items !== null &&  jFreshData?.items?.length>0 ? (
            <View style={styles.categoryView}>
              <FlatList
                contentContainerStyle={{
                  marginTop: wp(4),
                }}
                extraData={jFreshData?.items}
                data={ jFreshData?.items}
              ...
              { jFreshData !== null &&  jFreshData?.items !== null &&  jFreshData?.items?.length>0 ? (
                <View style={styles.categoryView}>
                  ...
                  >
                    ...
                    >
                      Subtotal ({jFreshData?.items.length} Items) :
                    </Text>
                    <Text
                      style={{
                        color: COLORS.BLACK,
                    ...
                    >
                      KES {jFreshData?.subTotal.toLocaleString()}
                    </Text>
                  </View>
                  <Button
                    style={{
                    ...
                    handlePress={() => navigation.navigate(NAVIGATION.checkOut,{
                      status:0,
                      isFresh:1
                    })}
                  />
